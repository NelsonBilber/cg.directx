...
ref class Shader sealed : public IFrameworkView
{
...
public:
	...
	virtual void Initialize(CoreApplicationView^ appView)
	{
		...
		appView->Activated += ref new TypedEventHandler<CoreApplicationView ^, IActivatedEventArgs ^>(this, &Shader::OnActivated);

		didCloseWindow = false;

		CoreApplication::Suspending += ref new EventHandler<SuspendingEventArgs ^>(this, &Shader::OnSuspending);
		CoreApplication::Resuming	+= ref new EventHandler<Platform::Object ^>(this, &Shader::OnResuming);
	}

	virtual void SetWindow(CoreWindow^ Window)
	{
		//tell windows to precess this kind os events
		Window->PointerPressed		+= ref new TypedEventHandler<CoreWindow ^, PointerEventArgs ^>(this, &Shader::PointerPressed);
		Window->PointerWheelChanged += ref new TypedEventHandler<CoreWindow ^, PointerEventArgs ^>(this, &Shader::OnPointerWheelChanged);
		Window->KeyDown				+= ref new TypedEventHandler<CoreWindow ^,KeyEventArgs ^>(this, &Shader::OnKeyDown);
		Window->Closed				+= ref new TypedEventHandler<CoreWindow ^, CoreWindowEventArgs ^>(this, &Shader::OnClosed);
	}
